    ...
    >
      ...
      <div className="relative aspect-4/3 w-full bg-[var(--bg-surface-secondary)] rounded-[var(--radius-lg,12px)] overflow-hidden flex items-center justify-center border border-[var(--border-subtle)]">
        ...
        {images[currentImgIdx] && (
          <button
            type="button"
            className="absolute top-3.5 right-3.5 bg-[var(--bg-surface)]/90 hover:bg-[var(--bg-surface)] text-[var(--text-primary)] p-2 rounded-[var(--radius-btn,6px)] transition-all shadow-xs border border-[var(--border-subtle)] cursor-pointer z-10"
      ...
      {images.length > 1 && (
        <div className="flex items-center gap-2 sm:gap-3">
          <button
            type="button"
            className="w-9 h-9 sm:w-10 sm:h-10 shrink-0 rounded-[var(--radius-btn,6px)] border border-[var(--border-default)] bg-[var(--bg-surface)] text-[var(--text-secondary)] hover:bg-[var(--bg-surface-secondary)] hover:text-[var(--text-primary)] flex items-center justify-center transition-all shadow-2xs cursor-pointer"
          ...
          <div className="flex-1 flex gap-2 overflow-x-auto py-1 scrollbar-none justify-center">
            {images.map((src, i) => (
              <button
                key={i}
                type="button"
                className={`w-13 h-13 sm:w-16 sm:h-16 shrink-0 rounded-[var(--radius-md,6px)] overflow-hidden border-2 p-1 transition-all bg-[var(--bg-surface)] cursor-pointer ${
                  i === currentImgIdx
      ...
      <div className="grid grid-cols-3 gap-2 pt-4 border-t border-[var(--border-subtle)]">
        <div className="flex flex-col sm:flex-row items-center justify-center gap-1.5 p-2 rounded-[var(--radius-md,6px)] bg-[var(--bg-surface-secondary)] border border-[var(--border-subtle)] text-center">
          <Icon icon="solar:verified-check-linear" className="w-4 h-4 text-[var(--brand-primary)] shrink-0" />
          <span className="text-[11px] font-bold text-[var(--text-primary)]">ISPM-15 Exempt</span>
        </div>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-1.5 p-2 rounded-[var(--radius-md,6px)] bg-[var(--bg-surface-secondary)] border border-[var(--border-subtle)] text-center">
          <Icon icon="solar:leaf-linear" className="w-4 h-4 text-[var(--brand-primary)] shrink-0" />
          <span className="text-[11px] font-bold text-[var(--text-primary)]">Zero Rot/Splinter</span>
        </div>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-1.5 p-2 rounded-[var(--radius-md,6px)] bg-[var(--bg-surface-secondary)] border border-[var(--border-subtle)] text-center">
          <Icon icon="solar:refresh-circle-linear" className="w-4 h-4 text-[var(--brand-primary)] shrink-0" />
          <span className="text-[11px] font-bold text-[var(--text-primary)]">100% Recycled PE</span>
        </div>
      </div>
    </div>
  );
