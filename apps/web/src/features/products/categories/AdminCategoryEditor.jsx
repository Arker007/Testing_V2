  ...
  useEffect(() => {
    ...
    setHeaderActions(
      <button
        ...
        style={{ minWidth: 150 }}
      >
        {saving ? <><Icon icon="carbon:renew" className="w-4 h-4 animate-spin mr-1 inline" /> Syncing…</> : <><Icon icon="carbon:save" className="w-4 h-4 mr-1 inline" /> Save Category</>}
      </button>
    );
        ...
        <form id="category-editor-form" onSubmit={handleSave}>
          <div className={styles.formGrid} style={{ gridTemplateColumns: "1.4fr 0.6fr" }}>
            <div>
              ...
              <button type="button" className={styles.actionBtnSecondary} onClick={() => setForm(p => ({ ...p, fields: [...p.fields, { name: "", label: "", type: "text", placeholder: "" }] }))}>
                <Icon icon="carbon:add-alt" className="w-4 h-4 mr-1 inline" /> Add Custom Field
              </button>
            </div>
                ...
                <div className={styles.imgGrid}>
                  ...
                  {!form.image && (
                    <label className={styles.imgAdd} style={{ width: "160px", height: "120px" }}>
                      ...
                      {uploading ? <Icon icon="carbon:renew" className="w-6 h-6 animate-spin" /> : <Icon icon="carbon:add-alt" className="w-6 h-6" />}
          ...
          <div className={styles.modalFooter} style={{ borderTop: "1px solid var(--gray-200)", paddingTop: "20px", marginTop: "32px" }}>
            ...
            <button type="submit" className={styles.actionBtnPrimary} disabled={saving || uploading} style={{ minWidth: "160px" }}>
              {saving ? <><Icon icon="carbon:renew" className="w-4 h-4 animate-spin mr-1 inline" /> Saving category...</> : <><Icon icon="carbon:save" className="w-4 h-4 mr-1 inline" /> Save Category</>}
            </button>
          </div>
        </form>
      </div>
