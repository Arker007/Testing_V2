import React, { useState } from "react";
import { CheckCircle, AlertTriangle, Loader2, Package, User, Mail, Globe, Phone, MessageSquare, Send, ShieldCheck, Headphones, MapPin, Navigation, ExternalLink } from "lucide-react";
import { useSite } from "../../shared/context/SiteContext";
import { ContactTrustedRow } from "./ContactTrustedRow";
import { useContactForm } from "./useContactForm";
import styles from "../../pages/Contact.module.css";

export default function ContactFormSection() {
  const { c, co } = useSite();
  const {
    form,
    status,
    handleChange: f,
    handleSubmit,
    resetStatus,
  } = useContactForm();

  const [showMobile, setShowMobile] = useState(false);

  // Fallbacks matching the website copy specs
  const contactPerson = c("contact_person", "Mr. Harsh Maru");
  const address = c("contact_address", "Plot No. 45-B, GIDC Industrial Estate, Ankleshwar, Gujarat 393002, India");
  const phoneVal = co("phone", "+91 98986 86379");
  const emailVal = c("contact_email", "info@vishalenterprise.com");

  return (
    <section id="contact-form-anchor" className={styles.mainSection}>
      {/* Decorative slant backgrounds */}
      <div className={styles.bgSlantBright} />
      <div className={styles.bgSlantDark} />
      <div className={styles.dotsPattern} />

      <div className="container relative z-10">
        <div className={styles.grid}>
          {/* Left Column: Interactive Enquiry Form */}
          <div className={styles.enquiryCard} id="enquiry-card">
            <div className={styles.formHeader}>
              <div className={styles.formEyebrowContainer}>
                <span className={styles.formEyebrow}>We're Here to Help</span>
                <div className={styles.formEyebrowLine} />
              </div>
              <h2 className={styles.formTitle}>
                Quick <span className={styles.greenText}>Enquiry</span>
              </h2>
              <p className={styles.formSubText}>
                Have a question or need a quote? Send us your details and we'll get back to you shortly.
              </p>
            </div>

            {status === "sent" ? (
              <div className={styles.successBox}>
                <CheckCircle className="w-16 h-16 text-[#98d12a]" />
                <h3>Inquiry Sent Successfully!</h3>
                <p>
                  Thank you for contacting us. Our sales team will get back to you within{" "}
                  <strong>2 business hours</strong> with pricing and spec sheets.
                </p>
                <button
                  type="button"
                  className={styles.successBtn}
                  onClick={resetStatus}
                >
                  Send Another Enquiry
                </button>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className={styles.contactForm}>
                {/* Product/Service & Name */}
                <div className={styles.row2}>
                  <div className={styles.inputFieldWrapper}>
                    <div className={styles.inputIcon}>
                      <Package className="w-5 h-5" />
                    </div>
                    <input
                      id="productService"
                      required
                      className={styles.customInput}
                      value={form.productService || ""}
                      onChange={f("productService")}
                      placeholder="Product / Service Looking for"
                    />
                  </div>

                  <div className={styles.inputFieldWrapper}>
                    <div className={styles.inputIcon}>
                      <User className="w-5 h-5" />
                    </div>
                    <input
                      id="fullName"
                      required
                      className={styles.customInput}
                      value={form.fullName || ""}
                      onChange={f("fullName")}
                      placeholder="Your Name"
                    />
                  </div>
                </div>

                {/* Email & Country */}
                <div className={styles.row2}>
                  <div className={styles.inputFieldWrapper}>
                    <div className={styles.inputIcon}>
                      <Mail className="w-5 h-5" />
                    </div>
                    <input
                      id="email"
                      type="email"
                      required
                      className={styles.customInput}
                      value={form.email || ""}
                      onChange={f("email")}
                      placeholder="Email"
                    />
                  </div>

                  <div className={styles.inputFieldWrapper}>
                    <div className={styles.inputIcon}>
                      <Globe className="w-5 h-5" />
                    </div>
                    <select
                      id="country"
                      required
                      className={styles.customSelect}
                      value={form.country || "India"}
                      onChange={f("country")}
                    >
                      <option value="India">India</option>
                      <option value="United States">United States</option>
                      <option value="United Kingdom">United Kingdom</option>
                      <option value="United Arab Emirates">United Arab Emirates</option>
                      <option value="Saudi Arabia">Saudi Arabia</option>
                      <option value="Singapore">Singapore</option>
                      <option value="Germany">Germany</option>
                      <option value="Canada">Canada</option>
                      <option value="Australia">Australia</option>
                    </select>
                  </div>
                </div>

                {/* Phone prefix and input */}
                <div className={styles.phoneInputRow}>
                  <div>
                    <select
                      id="phonePrefix"
                      className={styles.phonePrefixSelect}
                      value={form.phonePrefix || "+91"}
                      onChange={f("phonePrefix")}
                    >
                      <option value="+91">+91</option>
                      <option value="+1">+1</option>
                      <option value="+44">+44</option>
                      <option value="+971">+971</option>
                      <option value="+966">+966</option>
                      <option value="+65">+65</option>
                      <option value="+49">+49</option>
                      <option value="+61">+61</option>
                    </select>
                  </div>
                  <div className={styles.inputFieldWrapper}>
                    <div className={styles.inputIcon}>
                      <Phone className="w-5 h-5" />
                    </div>
                    <input
                      id="phone"
                      type="tel"
                      required
                      className={styles.customInput}
                      value={form.phone || ""}
                      onChange={f("phone")}
                      placeholder="Phone / Mobile"
                    />
                  </div>
                </div>

                {/* Message */}
                <div className={styles.inputFieldWrapper}>
                  <div className={styles.inputIcon} style={{ top: "1rem" }}>
                    <MessageSquare className="w-5 h-5" />
                  </div>
                  <textarea
                    id="message"
                    required
                    className={styles.customTextarea}
                    style={{ paddingLeft: "2.75rem" }}
                    rows={4}
                    value={form.message || ""}
                    onChange={f("message")}
                    placeholder="Leave a Message for us"
                  />
                </div>

                {status === "error" && (
                  <p className={styles.errorText} role="alert">
                    <AlertTriangle className="w-5 h-5 text-red-500 shrink-0" />
                    Sending failed. Please try again or contact us directly.
                  </p>
                )}

                <div className={styles.actionArea}>
                  <button
                    type="submit"
                    className={`${styles.submitBtn} swipe-btn magic-shimmer-btn`}
                    disabled={status === "sending"}
                    id="submit-message-btn"
                  >
                    {status === "sending" ? (
                      <>
                        <Loader2 className="w-5 h-5 animate-spin" />
                        <span className="btn-text">Sending Message...</span>
                      </>
                    ) : (
                      <>
                        <span className="btn-text">Send Message</span>
                        <span className="btn-icon-bubble">
                          <Send className="btn-arrow-icon w-4 h-4" />
                        </span>
                      </>
                    )}
                  </button>

                  <div className={styles.privacyNote}>
                    <ShieldCheck className={styles.privacyIcon} />
                    <span>We respect your privacy. Your information is safe with us.</span>
                  </div>
                </div>
              </form>
            )}
          </div>

          {/* Right Column: Contact Details & Info Cards */}
          <div id="contact-details-column">
            <div className={styles.rightTitleSection}>
              <span className={styles.rightEyebrow}>Get in Touch</span>
              <h2 className={styles.rightTitle}>
                Contact <span className={styles.greenText}>Vishal</span> Enterprise
              </h2>
              <div className={styles.rightUnderline} />
            </div>

            {/* Info Cards List */}
            <div className={styles.infoCardsGrid}>
              {/* Card 1: Contact Person */}
              <div className={styles.infoCardAccent} id="info-card-person">
                <div className={styles.infoCardIconCircle}>
                  <User className="w-5 h-5" />
                </div>
                <div className={styles.infoCardContent}>
                  <span className={styles.infoCardLabel}>Contact Person</span>
                  <p className={styles.infoCardValue}>{contactPerson}</p>
                </div>
              </div>

              {/* Card 2: Address */}
              <div className={styles.infoCardAccent} id="info-card-address">
                <div className={styles.infoCardIconCircle}>
                  <MapPin className="w-5 h-5" />
                </div>
                <div className={styles.infoCardContent}>
                  <span className={styles.infoCardLabel}>Address</span>
                  <p className={styles.infoCardValue}>{address}</p>
                </div>
              </div>

              {/* Card 3: Mobile */}
              <div className={styles.infoCardAccent} id="info-card-mobile">
                <div className={styles.infoCardIconCircle}>
                  <Phone className="w-5 h-5" />
                </div>
                <div className={styles.infoCardContent}>
                  <span className={styles.infoCardLabel}>Mobile</span>
                  {showMobile ? (
                    <p className={styles.infoCardValue}>
                      <a href={`tel:${phoneVal.replace(/\s+/g, "")}`} className="hover:underline text-[#98d12a] font-semibold">
                        {phoneVal}
                      </a>
                    </p>
                  ) : (
                    <button
                      type="button"
                      className={styles.infoCardLink}
                      onClick={() => setShowMobile(true)}
                    >
                      View Mobile Number
                    </button>
                  )}
                </div>
              </div>

              {/* Card 4: Email */}
              <div className={styles.infoCardAccent} id="info-card-email">
                <div className={styles.infoCardIconCircle}>
                  <Mail className="w-5 h-5" />
                </div>
                <div className={styles.infoCardContent}>
                  <span className={styles.infoCardLabel}>Email</span>
                  <p className={styles.infoCardValue}>
                    <a href={`mailto:${emailVal}`} className="hover:underline text-[#98d12a] font-semibold">
                      {emailVal}
                    </a>
                  </p>
                </div>
              </div>
            </div>

            {/* Immediate Assistance Banner */}
            <div className={styles.assistanceBanner} id="assistance-banner">
              <div className={styles.bannerLeft}>
                <div className={styles.bannerIconCircle}>
                  <Headphones className="w-5 h-5" />
                </div>
                <div>
                  <h4 className={styles.bannerTitle}>Need Immediate Assistance?</h4>
                  <p className={styles.bannerSub}>Our team is ready to help you with your requirements.</p>
                </div>
              </div>
              <a href={`tel:${phoneVal.replace(/\s+/g, "")}`} className={styles.bannerBtn}>
                <Phone className="w-4 h-4" />
                Call Now
              </a>
            </div>
          </div>
        </div>

        {/* Embedded Map Section */}
        <div className={styles.mapSectionWrapper} id="embedded-map-container">
          <div className={styles.mapHeader}>
            <div className={styles.mapTitleGroup}>
              <div className={styles.mapIconCircle}>
                <MapPin className="w-5 h-5" />
              </div>
              <div>
                <h3 className={styles.mapTitle}>Find Our Manufacturing Facility</h3>
                <p className={styles.mapAddress}>{address}</p>
              </div>
            </div>
            <a
              href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(address)}`}
              target="_blank"
              rel="noopener noreferrer"
              className={styles.mapDirectionsBtn}
            >
              <Navigation className="w-4 h-4" />
              <span>Get Directions</span>
              <ExternalLink className="w-3.5 h-3.5 opacity-70" />
            </a>
          </div>
          <div className={styles.mapIframeFrame}>
            <iframe
              title="Factory Location Map"
              src={`https://maps.google.com/maps?q=${encodeURIComponent(address)}&t=&z=15&ie=UTF8&iwloc=&output=embed`}
              className={styles.mapIframe}
              allowFullScreen=""
              loading="lazy"
              referrerPolicy="no-referrer"
            />
          </div>
        </div>

        {/* Dynamic Trusted Row below form if needed */}
        <ContactTrustedRow />
      </div>
    </section>
  );
}
